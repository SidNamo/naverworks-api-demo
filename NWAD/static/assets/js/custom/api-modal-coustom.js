...
var ModalScenarioAdd = function () {
				...
				validator.validate().then(function (status) {
					...
					if (status == 'Valid') {
						...
						fetch(url, {
							...
							if(res.flag == 0){
								Swal.fire({
									html: `<strong class="fs-4">Scenario 저장 확인</strong><br /><span class="fs-6">Scenario 저장이 정상적으로 완료되었습니다.</span>`,
									icon: "success",
									buttonsStyling: false,
									confirmButtonText: "확인",
									customClass: {
	...
	return {
		...
		init: function () {
			// Elements
			modalEl = document.querySelector('#sm_sc_save');
			...
			form = modalEl.querySelector('#sm_sc_save_form');
			submitButton = modalEl.querySelector('#sm_sc_info_submit');
			cancelButton = modalEl.querySelector('#sm_sc_info_cancel');
